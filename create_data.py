...
def length_filter(dictionary: list[str], ngram_length: int | None = None, mode: str | None = None) -> list[str]:
    """
    ...
    """
    if mode is None and ngram_length is None:
        raise ValueError('Error: Must specify word_length if mode is not specified')
    new_dictionary = []
    if mode == 'prefix':
        for word in dictionary:
        ...
        for word in dictionary:
            word_right_padded = word + '_' * (ngram_length - len(word))
            new_dictionary.append(word_right_padded)
        new_dictionary = np.random.choice(new_dictionary,20000) #MoE runs slow so I wanted smaller sample
...
def encode_function() -> Callable[[str], list[int]]:
    ...
    """
    chars = ['.'] + list(string.ascii_lowercase) + ['_'] # the '.' token represents an unknown letter while '_' is padding
...
class TrainData(Dataset): 
    ...
    # Constructor
    def __init__(self, block_size: int, words: list[str],mode: str | None = None):
...
def create_and_save_dataset(block_size: int, words: list[str], save_location: str, mode: str | None = None) -> None:
    """
