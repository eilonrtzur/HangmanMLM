...
def remove_guessed(model_output: torch.Tensor, guessed_letters: list[int]) -> torch.Tensor:
    ...
    if torch.sum(model_output) != 0:
        model_output = model_output/torch.sum(model_output) 
    return model_output

def generate_prediction(models: list[TransformerHangman], guessed_letters: list[str], status: str, weights: torch.Tensor) -> str:
...
def PlayHangman(word: str, weight: torch.Tensor = None, mode: str = None) -> int:
    """ 
    ...
    """
    if weight == None:
        weight = torch.ones(min(len(word),8))
    else:
...
def WinRate(sample: list[str], weight: torch.Tensor = None) -> float:
...
if __name__ == '__main__':
    ...
    if response == 'game':
        ...
        if word != 'random please':
            ...
            PlayHangman(create_sample('allwords.pkl',1)[0],None,'print')
